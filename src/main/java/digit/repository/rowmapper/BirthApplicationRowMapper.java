    ...
    public List<BirthRegistrationApplication> extractData(ResultSet rs) throws SQLException, DataAccessException {
        ...
        while (rs.next()) {
            ...
            if (birthRegistrationApplication == null) {
                ...
                // Handling timeOfBirth conversion properly
                LocalDateTime timeOfBirth = null;
                int dobInt = rs.getInt("btimeofbirth");
                String dobStr = String.valueOf(dobInt); // Convert to string

                if (dobStr.length() == 8) { // Ensure valid format
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("ddMMyyyy", Locale.ENGLISH);
                    LocalDate birthDate = LocalDate.parse(dobStr, formatter); // Convert to LocalDate
                    timeOfBirth = birthDate.atStartOfDay(); // Convert to LocalDateTime
                }
                ...
                birthRegistrationApplication = BirthRegistrationApplication.builder()
                        ...
                        .timeOfBirth(timeOfBirth) // Corrected conversion
